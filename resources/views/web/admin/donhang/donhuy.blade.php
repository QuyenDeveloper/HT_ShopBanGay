...
@section('content')
    ...
    <div>
        <?php
        $TSD = 0;
        foreach ($donhuys as $donhuy){
            $TSD++;
        }
        ?>
        <div style="text-align: right; padding-right: 30px;">
            Tổng số đơn bị hủy: <span style="color: red; font-size: 30px;">{{$TSD}}</span>
        </div>
                ...
                <div style="background: rgba(224,220,220,0.24); border-radius: 5px;">
                    <div class="CXN" >
                        @foreach($ngdungs as $ngdung)
                            @if($donhuy->idKH == $ngdung->id)
                                <span>Tên tài khoản:</span><span> {{$ngdung->name}} </span><br><br>
                            @endif
                        @endforeach
                        <span>THÔNG TIN ĐƠN HÀNG:</span>
                        @foreach($diachis as $diachi)
                            @if($donhuy->idDchi == $diachi->id)
                                <table>
                                    <tr>
                                        <th><span>Mã đơn:</span></th> <td><span style="color: red;">{{$donhuy->idDMua}}</span></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <th><span>Lời nhắn:</span></th><td><span style="color: red;">{{$donhuy->loinhan}}</span></td>
                                    </tr>
                                    <tr>
                                        <th><span>Loại địa chỉ:</span></th><td><span style="color: red;">{{$diachi->loaiDC}}</span></td>
                                    </tr>
                                    <tr>
                                        <th><span>Thời gian lên đơn:</span></th><td><span style="color: red;">{{$donhuy->created_at}}</span></td>
                                    </tr>
                                </table>
                            @endif
                        @endforeach
                    ...
                    @foreach($donmuas as $donmua)
                        @if($donhuy->idDMua === $donmua->idDMua)
                            @foreach($sps as $sp)
