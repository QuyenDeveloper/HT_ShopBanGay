...
@section('content')
    ...
    <div>
        <?php
        $TSD = 0;
        foreach ($dagiaos as $dagiao){
            $TSD++;
        }
        ?>
        <div style="text-align: right; padding-right: 30px;">
            Tổng số đơn đã giao: <span style="color: red; font-size: 30px;">{{$TSD}}</span>
        </div>
        ...
        @foreach($dagiaos as $dagiao)
            ...
            <div class="dh">
                <div style="background: rgba(224,220,220,0.24); border-radius: 5px;">
                    <div class="CXN" >
                        @foreach($ngdungs as $ngdung)
                            @if($dagiao->idKH == $ngdung->id)
                                <span>Tên tài khoản:</span><span> {{$ngdung->name}} </span><br><br>
                            @endif
                        @endforeach
                        <span>THÔNG TIN ĐƠN HÀNG:</span>
                        @foreach($diachis as $diachi)
                            @if($dagiao->idDchi == $diachi->id)
                                <table>
                                    <tr>
                                        <th><span>Mã đơn:</span></th> <td><span style="color: red;">{{$dagiao->idDMua}}</span></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <th><span>Lời nhắn:</span></th><td><span style="color: red;">{{$dagiao->loinhan}}</span></td>
                                    </tr>
                                    <tr>
                                        <th><span>Loại địa chỉ:</span></th><td><span style="color: red;">{{$diachi->loaiDC}}</span></td>
                                    </tr>
                                    <tr>
                                        <th><span>Thời gian lên đơn:</span></th><td><span style="color: red;">{{$dagiao->created_at}}</span></td>
                                    </tr>
                                </table>
                            @endif
                        @endforeach
                    ...
                    @foreach($donmuas as $donmua)
                        @if($dagiao->idDMua === $donmua->idDMua)
                            @foreach($sps as $sp)
                ...
                <div style="background: #f6f4ee; border-radius: 5px;">
                    <div class="tonggia">
                        Tổng số tiền: <span>{{$Ttien}} đ</span>
                    </div>
                </div>
            </div>
