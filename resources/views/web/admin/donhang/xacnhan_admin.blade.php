                ...
                <div class="dh">
                    <div style="background: rgba(224,220,220,0.24); border-radius: 5px;">
                        <div class="CXN" >
                            @foreach($ngdungs as $ngdung)
                                @if($xacnhan->idKH == $ngdung->id)
                                    <span>Tên tài khoản:</span><span> {{$ngdung->name}} </span><br><br>
                                @endif
                            @endforeach
                                <span>THÔNG TIN ĐƠN HÀNG:</span>
                            @foreach($diachis as $diachi)
                                @if($xacnhan->idDchi == $diachi->id)
                                    <table>
                                        <tr>
                                            <th><span>Mã đơn:</span></th> <td><span style="color: red;">{{$xacnhan->idDMua}}</span></td>
                                        </tr>
                                        <tr>
                                        ...
                                        <tr>
                                            <th><span>Lời nhắn:</span></th><td><span style="color: red;">{{$xacnhan->loinhan}}</span></td>
                                        </tr>
                                        <tr>
                                            <th><span>Loại địa chỉ:</span></th><td><span style="color: red;">{{$diachi->loaiDC}}</span></td>
                                        </tr>
                                        <tr>
                                            <th><span>Thời gian lên đơn:</span></th><td><span style="color: red;">{{$xacnhan->created_at}}</span></td>
                                        </tr>
                                    </table>
                                @endif
                            @endforeach
                        ...
                        @foreach($donmuas as $donmua)
                            @if($xacnhan->idDMua === $donmua->idDMua)
                                @foreach($sps as $sp)
                    ...
                    <div style="background: #f6f4ee; border-radius: 5px;">
                        ...
                        <div class="nut">
                            ...
                            <a href="/trangchu/admin/donhang/xl_xacnhan/{{$xacnhan->idDMua}}"><button>Xác nhận đơn</button></a>
                        </div>
