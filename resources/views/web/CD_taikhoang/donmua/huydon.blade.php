...
@section('donmua')
    <style>
        ...
        .nut  a.cho button:hover{
            background: rgba(255, 0, 0, 0.48);
        }
    </style>

    <div>
        @foreach($huydons as $huydon)
            <div class="dh">
                <div style="background: rgba(224,220,220,0.24); border-radius: 5px;">
                    <div class="CXN" >
                        <span style="color: red;">{{$huydon->idDMua}}</span> Đã hủy
                    </div>
                    <?php $tongtien =0; ?>
                    @foreach($donmuas as $donmua)
                        @if($huydon->idDMua === $donmua->idDMua)
                            @foreach($sps as $sp)
                ...
                <div style="background: #f6f4ee; border-radius: 5px;">
                    ...
                    <div class="nut">
                        <a href="#" class="cho"><button>mua lại </button></a>
